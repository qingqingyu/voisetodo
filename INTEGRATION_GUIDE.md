...
| Agent B | AI 提取：Cloudflare Worker 代理 + JSON 解析 + 重试策略 | ✅ 完成 |
...
不要在 iOS App 内配置 OpenAI 或 Anthropic API Key。先部署 `AIProxy/` 里的 Cloudflare Worker，并把真正的供应商 Key 配置为 Cloudflare Secrets。

```bash
VOICETODO_AI_PROXY_ENDPOINT=https://your-worker.workers.dev/v1/todo-extractions
VOICETODO_AI_PROXY_APP_TOKEN=your-app-token
```

iOS 端只保存代理 URL 和可轮换的 App Token，不接触 AI 供应商 Key。
...
**解决**：
- 检查 `VOICETODO_AI_PROXY_ENDPOINT` 是否指向已部署 Worker
- 检查 Worker Secrets 中的供应商 API Key 是否正确
- 检查 App Token、每日额度和 Worker 日志
- 检查网络连接
