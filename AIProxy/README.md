...
| Global budget (per tier) | `global-quota:<date>` (free) / `global-quota-pro:<date>` (pro) → 503 for **that tier only** when exceeded (pro error code `global_budget_exceeded_pro`); a free-bucket trip does not affect paid users | `GLOBAL_DAILY_LIMIT`, `GLOBAL_PRO_DAILY_LIMIT` + `RATE_LIMIT_KV` (pro limit unset = no global cap for paid; device `PAID_DAILY_LIMIT` still applies) |
