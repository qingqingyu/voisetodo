...
| Per-subscription daily (pro) | `sub-quota:<utc-date>:<originalTransactionId>` — the JWS is an unbound bearer token and `X-Device-ID` is client-chosen, so **one valid JWS + device-ID rotation** defeats every per-device counter; the subscription id from the verified JWS payload is the one identity rotation cannot change. UTC-day keyed (server truth, immune to `X-Local-Date` drift), no refund (cost-side guard, same policy as the global budget). Exceeding → 429 `subscription_quota_exceeded` + `Retry-After` to next UTC midnight; the iOS client maps it to "rate limited, retry later" (not paywall). Missing id in payload/cache → layer skipped with a log line (≤15 min post-deploy cache transition) | `SUBSCRIPTION_DAILY_LIMIT` (default 500 ≈ 5 devices maxing `PAID_DAILY_LIMIT`; 1–3 real devices never come close) |
| Per-IP daily / velocity (free tier only) | `ip-quota:<date>:<ipHash>`, `ip-rate:<minute>:<ipHash>` (independent of device → blocks `X-Device-ID` rotation; daily limit should remain well above the per-device quota for shared egress IPs). Verified Pro requests are exempt from both IP gates — CGNAT/VPN shared egress would otherwise throttle paying users, and multi-device abuse is bounded by the per-subscription cap above instead | `IP_DAILY_LIMIT`, `IP_RATE_PER_MINUTE` + `RATE_LIMIT_KV` |
| Global budget (per tier) | DO path: UTC-hour-bucket **rolling window** (`windowHours`, default 6h) with in-window threshold = `daily_limit × windowHours/24` — sustained abuse keeps the daily cost ceiling unchanged, while a single breach locks the tier for at most ~one window (the trip flag TTL expires at the next hour boundary + 120 s propagation margin, then the DO re-evaluates; old buckets rolling out is what un-trips it). 503 carries `Retry-After` = next hour boundary + 120 s (aligned with the trip flag TTL — earlier retries are guaranteed another 503). **Counted by all three modes** — extract and split/reflect both increment their tier's bucket (assist modes stay off the *billing* quota by product decision, but they are model calls, so they count toward the *cost* budget; before this, the pro trip flag was never set by assist traffic and pro split/reflect had no limit at all). Degraded KV path keeps the day-bucket semantics (`global-quota:<date>` free / `global-quota-pro:<date>` pro). A trip is per tier (pro error code `global_budget_exceeded_pro`); a free-bucket trip does not affect paid users | `GLOBAL_DAILY_LIMIT`, `GLOBAL_PRO_DAILY_LIMIT`, `GLOBAL_BUDGET_WINDOW_HOURS` + `RATE_LIMIT_KV`/`QUOTA_COUNTER_DO` (pro limit unset = no global cap for paid; device `PAID_DAILY_LIMIT` and the per-subscription cap still apply) |

**Recommended additional edge layer (configure in Cloudflare dashboard, not code):** add a **Rate Limiting Rule** on path `/v1/todo-extractions` keyed by `ip.src` (e.g. ≤ 10 req/min, ≤ 500 req/day per IP). Keep the daily IP ceiling well above the per-device quota because VPNs, carrier NAT, and company networks share egress IPs. Note an `ip.src`-keyed edge rule applies to all traffic including verified Pro requests — keep its ceilings generous enough not to re-introduce the shared-egress throttling the Worker-level Pro exemption exists to avoid. Cloudflare's native rate limiter handles bursts and IP rotation more efficiently than KV counters and runs before the Worker — use it as the first line, with the Worker-level limits above as defense-in-depth.

Per-tier (free/paid) limits are implemented: the client sends a verified `X-Subscription-JWS`, and both the billing quota and the anti-abuse gates above branch on it. Not yet implemented (future): App Attest / DeviceCheck to make `X-Device-ID` unforgeable.
