...
4. 提取时间并换算为绝对日期：结合参考日期，把提到的时间（明天、下周三、月底前）换算成 due_date（ISO 8601 "YYYY-MM-DD"）。禁止返回"明天"/"下周三"等相对表达——必须算出具体日期。没提到时间则 due_date 为 null。时间原文保留在 due_hint（供用户参考），但程序只用 due_date。若提到明确钟点（下午3点、晚上8点半、15:00），额外用 due_time 返回 24 小时制 "HH:mm"（下午3点→"15:00"、晚上8点半→"20:30"）；只有天级或没提到钟点则 due_time 为 null。一周从周一开始（ISO 8601 / Apple Calendar 约定），"下周三"=当前周之后的那个周三
...
示例 1（无时间线索）：
输入："帮我记一下买菜"
输出：{"todos":[{"title":"买菜","detail":"帮我记一下买菜","due_date":null,"due_hint":null,"due_time":null,"recurrence_rule":null,"recurrence_end":null,"priority":"normal","category_hint":"life"}],"ignored":""}

示例 2（带钟点，假设参考日期 2026-07-12 周日）：
输入："明天下午3点开会"
输出：{"todos":[{"title":"开会","detail":"明天下午3点","due_date":"2026-07-13","due_hint":"明天下午3点","due_time":"15:00","recurrence_rule":null,"recurrence_end":null,"priority":"normal","category_hint":"work"}],"ignored":""}

示例 3（月度重复 + 钟点）：
输入："每个月15号下午3点发工资提醒"
输出：{"todos":[{"title":"发工资提醒","detail":"每个月15号下午3点","due_date":null,"due_hint":"每个月15号下午3点","due_time":"15:00","recurrence_rule":{"frequency":"monthly","weekdays":[],"day_of_month":15,"end_date":null},"recurrence_end":null,"priority":"normal","category_hint":"finance"}],"ignored":""}

示例 4（周重复多天）：
输入："每周一三五晚上8点去健身房"
输出：{"todos":[{"title":"去健身房","detail":"每周一三五晚上8点","due_date":null,"due_hint":"每周一三五晚上8点","due_time":"20:00","recurrence_rule":{"frequency":"weekly","weekdays":[2,4,6],"day_of_month":null,"end_date":null},"recurrence_end":null,"priority":"normal","category_hint":"health"}],"ignored":""}

示例 5（有限周期 + 每天 + 钟点）：
输入："未来一个月每天下午3点来接孩子"
输出：{"todos":[{"title":"接孩子","detail":"未来一个月每天下午3点","due_date":null,"due_hint":"未来一个月每天下午3点","due_time":"15:00","recurrence_rule":{"frequency":"daily","weekdays":[],"day_of_month":null,"end_date":null},"recurrence_end":{"kind":"after_count","count":1,"unit":"month"},"priority":"normal","category_hint":"life"}],"ignored":""}

示例 6（重复 + 非"未来N"截止边界）：
输入："每天晚上8点吃药，到这个月底"
输出：{"todos":[{"title":"吃药","detail":"每天晚上8点吃药，到这个月底","due_date":null,"due_hint":"每天晚上8点","due_time":"20:00","recurrence_rule":{"frequency":"daily","weekdays":[],"day_of_month":null,"end_date":null},"recurrence_end":{"kind":"month_end","scope":"this"},"priority":"normal","category_hint":"health"}],"ignored":""}

示例 7（相对日期换算，假设参考日期 2026-07-12 周日）：
输入："下周三交房租"
输出：{"todos":[{"title":"交房租","detail":"下周三交房租","due_date":"2026-07-15","due_hint":"下周三","due_time":null,"recurrence_rule":null,"recurrence_end":null,"priority":"normal","category_hint":"finance"}],"ignored":""}

示例 8（N天后换算，假设参考日期 2026-07-12）：
输入："三天后下午3点开会"
输出：{"todos":[{"title":"开会","detail":"三天后下午3点开会","due_date":"2026-07-15","due_hint":"三天后下午3点","due_time":"15:00","recurrence_rule":null,"recurrence_end":null,"priority":"normal","category_hint":"work"}],"ignored":""}`;
...
4. Extract time cues and compute absolute dates: Using the reference date, convert any mentioned time (tomorrow, next Wednesday, by end of month) into an ISO 8601 absolute date as due_date ("YYYY-MM-DD"). NEVER return relative expressions like "tomorrow" or "next Wednesday" in due_date — always compute the concrete date. If no time is mentioned, due_date is null. Keep the original text in due_hint (for user context), but the app uses due_date for rendering. If a specific clock time is also mentioned (3pm, 8:30pm, 15:00), additionally return due_time as 24-hour "HH:mm" (3pm→"15:00", 8:30pm→"20:30"); if only day-level or no clock time, due_time is null. The week starts on Monday (ISO 8601 / Apple Calendar convention), so "next Wednesday" = the Wednesday of the following week
...
Output: {"todos":[{"title":"Buy groceries","detail":"Remind me to buy groceries","due_date":null,"due_hint":null,"due_time":null,"recurrence_rule":null,"recurrence_end":null,"priority":"normal","category_hint":"life"}],"ignored":""}

Example 2 (with clock time, assume reference date 2026-07-12 Sunday):
Input: "Meeting tomorrow at 3pm"
Output: {"todos":[{"title":"Meeting","detail":"tomorrow at 3pm","due_date":"2026-07-13","due_hint":"tomorrow at 3pm","due_time":"15:00","recurrence_rule":null,"recurrence_end":null,"priority":"normal","category_hint":"work"}],"ignored":""}

Example 3 (monthly recurrence + clock time):
Input: "Salary reminder on the 15th of every month at 3pm"
Output: {"todos":[{"title":"Salary reminder","detail":"15th of every month at 3pm","due_date":null,"due_hint":"15th of every month at 3pm","due_time":"15:00","recurrence_rule":{"frequency":"monthly","weekdays":[],"day_of_month":15,"end_date":null},"recurrence_end":null,"priority":"normal","category_hint":"finance"}],"ignored":""}

Example 4 (weekly recurrence, multiple weekdays):
Input: "Gym every Mon/Wed/Fri at 8pm"
Output: {"todos":[{"title":"Go to gym","detail":"every Mon/Wed/Fri at 8pm","due_date":null,"due_hint":"every Mon/Wed/Fri at 8pm","due_time":"20:00","recurrence_rule":{"frequency":"weekly","weekdays":[2,4,6],"day_of_month":null,"end_date":null},"recurrence_end":null,"priority":"normal","category_hint":"health"}],"ignored":""}
...
Output: {"todos":[{"title":"Take medicine","detail":"every day at 8pm until end of this month","due_date":null,"due_hint":"every day at 8pm","due_time":"20:00","recurrence_rule":{"frequency":"daily","weekdays":[],"day_of_month":null,"end_date":null},"recurrence_end":{"kind":"month_end","scope":"this"},"priority":"normal","category_hint":"health"}],"ignored":""}
...
Output: {"todos":[{"title":"Pay rent","detail":"pay the rent next Wednesday","due_date":"2026-07-15","due_hint":"next Wednesday","due_time":null,"recurrence_rule":null,"recurrence_end":null,"priority":"normal","category_hint":"finance"}],"ignored":""}

Example 8 (N days from now, assume reference date 2026-07-12):
Input: "I have a meeting at 3pm three days from now"
Output: {"todos":[{"title":"Meeting","detail":"meeting at 3pm three days from now","due_date":"2026-07-15","due_hint":"three days from now","due_time":"15:00","recurrence_rule":null,"recurrence_end":null,"priority":"normal","category_hint":"work"}],"ignored":""}`;
