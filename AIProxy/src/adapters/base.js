...
export function buildSystemPrompt(locale, vocabularyHints = [], today = "") {
  // today 是客户端时区日期（YYYY-MM-DD，已通过 resolveQuotaDate 漂移校验）。
  // AI 需要它来计算"未来 X 天/周/月"等有限周期的 end_date（YYYY-MM-DD）。
  // 没有今天日期，AI 只能返回 null end_date（"未来一个月每天"这种场景就算不出来）。
  const todayLine = today
    ? (locale === "zh" ? `\n\n今天的日期是 ${today}（YYYY-MM-DD，设备时区）。计算 end_date 等相对日期时以此为基准。` : `\n\nToday's date is ${today} (YYYY-MM-DD, device timezone). Use this as the reference when computing end_date or any relative dates.`)
    : "";
...
示例 5（有限周期 + 每天 + 钟点）：
输入："未来一个月每天下午3点来接孩子"
输出：{"todos":[{"title":"接孩子","detail":"未来一个月每天下午3点","due_hint":"未来一个月每天下午3点","due_time":"15:00","recurrence_rule":{"frequency":"daily","weekdays":[],"day_of_month":null,"end_date":"<今天的日期 + 1 个月，YYYY-MM-DD>"},"priority":"normal","category_hint":"life"}],"ignored":""}

注意：示例 5 的 end_date 必须是基于系统消息中"今天的日期"实际计算出的具体 YYYY-MM-DD（如今天 2026-07-05，则 end_date=2026-08-05），不要返回占位符或 null`;
...
Output: {"todos":[{"title":"Pick up the kid","detail":"every day at 3pm for the next month","due_hint":"every day at 3pm for the next month","due_time":"15:00","recurrence_rule":{"frequency":"daily","weekdays":[],"day_of_month":null,"end_date":"<today's date + 1 month, YYYY-MM-DD>"},"priority":"normal","category_hint":"life"}],"ignored":""}

Note: Example 5's end_date must be a concrete YYYY-MM-DD computed from "Today's date" in the system message (e.g. if today is 2026-07-05, end_date=2026-08-05). Do not return a placeholder or null.`;
