...
5. 提取重复规则：只有明确出现「每天/每日/每周X/每月X号」时才设置 recurrence_rule；否则为 null。weekdays 编号遵循 Apple Calendar 约定：1=周日、2=周一、3=周二、4=周三、5=周四、6=周五、7=周六（注意不是 ISO 8601 的 1=周一）。例如「每周一三五」→ weekdays=[2,4,6]；「每月15号」→ frequency="monthly", day_of_month=15。若出现「未来7天/接下来7天/连续7天」这类有限周期，end_date 用 YYYY-MM-DD 表示最后一次发生日期；无法确定具体日期时设为 null
...
示例 1（无时间线索）：
输入："帮我记一下买菜"
输出：{"todos":[{"title":"买菜","detail":"原话","due_hint":null,"due_time":null,"recurrence_rule":null,"priority":"normal","category_hint":"life"}],"ignored":""}
...
5. Extract recurrence only for explicit phrases like "every day", "daily", "every Monday", "weekly", or "monthly on the 1st"; otherwise recurrence_rule must be null. weekdays numbering follows Apple Calendar convention: 1=Sunday, 2=Monday, 3=Tuesday, 4=Wednesday, 5=Thursday, 6=Friday, 7=Saturday (NOT ISO 8601 where 1=Monday). E.g. "every Mon/Wed/Fri" → weekdays=[2,4,6]; "monthly on the 15th" → frequency="monthly", day_of_month=15. For bounded phrases like "for the next 7 days", set end_date to the final occurrence date in YYYY-MM-DD when the date can be determined; otherwise use null
...
Output: {"todos":[{"title":"Buy groceries","detail":"original quote","due_hint":null,"due_time":null,"recurrence_rule":null,"priority":"normal","category_hint":"life"}],"ignored":""}
