...
export function buildSystemPrompt(locale, vocabularyHints = [], today) {
  ...
  // 漂移校验），缺失/非法时回退服务端 UTC。AI 需要它来计算"未来 X 天/周/月"等有限
  // 周期的 end_date。没有 today，AI 只能返回 null end_date。
  // 注意：当 today 来自服务端 UTC 回退时，与用户真实"今天"可能差 1 天（跨时区场景），
  //       iOS 端 RecurrenceRule 解析是第二道防线。
  ...
  if (locale === "zh") {
    ...
    todayLine = `\n\nReference date: ${today} (YYYY-MM-DD). Use this as the base when computing end_date or any relative dates.`;
  }
...
// System prompts below carry 5 few-shot examples each (added 2026-07) to anchor
// weekday numbering, the required-ignored rule, and bounded-period end_date
// computation. Approximate input token cost per request: ~660 tokens (zh prompt)
// / ~660 tokens (en prompt); only one locale is sent per request. iOS side
// (ExtractedTodo decoder + ignored:null fallback in commit 5dd5b4f) remains the
// second line of defense — do NOT assume the prompt alone guarantees AI never
// returns null.
...
5. 提取重复规则：只有明确出现「每天/每日/每周X/每月X号」时才设置 recurrence_rule；否则为 null。weekdays 编号映射表（Apple Calendar 约定，与 iOS RecurrenceRule.swift 一致）：周日=1、周一=2、周二=3、周三=4、周四=5、周五=6、周六=7。例如「每周一三五」→ weekdays=[2,4,6]；「每月15号」→ frequency="monthly", day_of_month=15。若出现「未来X天/接下来X周/连续X月/未来一个月」这类有限周期，end_date 用 YYYY-MM-DD 表示最后一次发生日期（基于系统消息中的"参考日期"计算）；无法确定具体日期时设为 null
...
示例 5（有限周期 + 每天 + 钟点；假设参考日期为 2026-07-05）：
输入："未来一个月每天下午3点来接孩子"
输出：{"todos":[{"title":"接孩子","detail":"未来一个月每天下午3点","due_hint":"未来一个月每天下午3点","due_time":"15:00","recurrence_rule":{"frequency":"daily","weekdays":[],"day_of_month":null,"end_date":"2026-08-05"},"priority":"normal","category_hint":"life"}],"ignored":""}

注意：示例 5 的 end_date 是基于系统消息中"参考日期"+1 个月的实际计算结果（参考日期变化时 end_date 相应变化，如参考日期 2026-07-05 → end_date 2026-08-05）。不要返回占位符或 null`;
...
5. Extract recurrence only for explicit phrases like "every day", "daily", "every Monday", "weekly", or "monthly on the 1st"; otherwise recurrence_rule must be null. weekdays numbering map (Apple Calendar convention, matches iOS RecurrenceRule.swift): Sunday=1, Monday=2, Tuesday=3, Wednesday=4, Thursday=5, Friday=6, Saturday=7. E.g. "every Mon/Wed/Fri" → weekdays=[2,4,6]; "monthly on the 15th" → frequency="monthly", day_of_month=15. For bounded phrases like "for the next X days/weeks/months" or "next month", set end_date to the final occurrence date in YYYY-MM-DD (computed from "Reference date" in the system message); use null only when the date cannot be determined
...
Example 5 (bounded period + daily + clock time; assume reference date is 2026-07-05):
Input: "Pick up the kid every day at 3pm for the next month"
Output: {"todos":[{"title":"Pick up the kid","detail":"every day at 3pm for the next month","due_hint":"every day at 3pm for the next month","due_time":"15:00","recurrence_rule":{"frequency":"daily","weekdays":[],"day_of_month":null,"end_date":"2026-08-05"},"priority":"normal","category_hint":"life"}],"ignored":""}

Note: Example 5's end_date is the reference date + 1 month, computed from "Reference date" in the system message (the date shifts with the reference date, e.g. reference 2026-07-05 → end_date 2026-08-05). Do not return a placeholder or null.`;
