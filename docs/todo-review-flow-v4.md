...
> 状态：**方案待拍板**（2026-09-20，未实施）。基线 `cab54b3`（main）。开发分支 `claude/review-flow-decision-output-959xqn`。
...
| # | 决定 | 建议 | 与既有拍板对账 |
|---|---|---|---|
| 1 | 地板层是否走冷却 | **不走** | §2.4 冷却是为「同一句判断反复说教」设计的；事实每期都该重述（积压这周就是比上周老 7 天）。不改冷却对规则层的作用 |
| 2 | 冷却本身是否放宽 | **本轮不动**，先观察地板层上线后是否还缺 | 动 §2.4 影响四条规则全体，且 `shownInsights` 历史语义连带；地板层已能兜住空屏，没必要同轮改两处 |
| 3 | 03 中间地带是否降级成一行事实 | **是**（只报占比，不带判断） | 与 §2.2「文案止于观察」一致；但**明确推翻** `ReactiveVsPlannedRule.swift:67`「无可行动信号,不显示」的原判断——理由：一屏什么都不说比说一个中性事实更糟，且该事实是第 5 步「问问自己」的素材 |
| 4 | `InsightOpenTask` 加 `category` | **是** | §1.4「原料级 DTO 不替规则做形状设计」——加字段属原料级，不违 |
| 5 | 降级阶梯（ladder）改口 | **是，且这条最要紧** | v1 §2.3 的 `<5 完成记录 → 整步跳过` 隐含假设「**洞察 = 完成记录的函数**」。地板 A 只依赖 `openTasks`，不依赖完成记录——一个从没完成过任何任务、但攒了 19 条积压的用户，恰恰最需要看到积压年龄。**建议 `skipStep` 档改为「只出地板 A」**，整步跳过只保留给「零积压 + 零完成」 |

拍板 5 连带：`skipsInsightsWhenEmpty`（闸 4）在地板层上线后基本恒为 false，判定应改写成「**地板层与规则层是否都空**」，而不是现在的「有没有占位文案」。占位行同步降级为地板块下方的脚注，不再单独撑起一屏。
...
| 2 | 拍板 5：ladder 改口 + `skipsInsightsWhenEmpty` 判定改写 + 占位行降级为脚注 | 依赖批 1 |
| 3 | 拍板 4 + 地板 C（`InsightOpenTask.category`）；拍板 3（03 中间地带事实行） | 依赖拍板 |
