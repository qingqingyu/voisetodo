...
## 实施时补全的微观决策（review 补充 2026-08-08）

> 本节由 review 在 2026-08-08 补充。前面「核心设计」+「实施步骤」是已拍板的设计决策，本节是**实施过程中必然遇到、但文档没说透**的 UX / 边界细节。每条给一个推荐做法，可调整。

### 联动清除：清空开始日期时自动清掉结束日期

Step 6 没说联动。用户场景：先设了"8/14 → 8/16"，然后把开始日期清空。

- 数据层不变式 1（行 109：`dueDate == nil ⇒ eventEndDate = nil`）会在归一化时自动清掉 `eventEndDate` —— 这层是安全的
- UI 层建议：在详情页编辑态监听 `editedDueDate`，一旦变成 `nil`，立即把 `editedEventEndDate` 也设为 `nil`，并给一个微动效提示"跨天区间已清除"
- 反过来：用户清掉结束日期不影响开始日期

### 非起始日标题前缀的 RTL 行为

Step 5 提到"非起始日在标题前加连续标记（`↳` 或 `→`）"。CLAUDE.md 的 RTL 规则要求方向符号加 `.flipsForRightToLeftLayoutDirection(true)` 或避开方向符号。

**建议改为纯文本标记**，完全避开方向问题：

- 起始日：标题正常显示
- 中间日 / 结束日：标题前缀显示 `"第 N/M 天 "`（复用 Step 9 的 `span.day_progress_format` 文案）
- 月历格子里的连续性靠"标题文本一致 + 钟点前缀只在起始日显示"暗示，不用方向符号

这样在 LTR / RTL 下表现一致。

### 跨天事件多的月历"满负载"测试

Step 5 说"条数 / 行高不变量本次不动"。但月历每天能塞下的 event 数是有限的，多个跨天事件叠加可能溢出。

**必测场景**：一个月内有 3+ 个并发的跨天事件（如"周五–周日出游" + "周三–周四 会议" + "12–15 号 团建"），观察：

- 月历格子是否挤爆 / 截断到只剩 "+N"
- 是否影响 `HomeMonthHeaderView.allocateRowHeights`（如果影响了，本次要破例修）

如果"满负载"场景挤爆严重，**建议本次破例加一次 row height 分配器调整**（虽然约束说不动），因为不修等于功能残废。

### 跨度上限截断的 UI 提示

不变式 4（行 112）：跨度上限 366 天，越界则截断。

- 数据层归一化截断是静默的
- 导入路径（`SystemCalendarEventImporter`）若一个事件跨年（比如休假 400 天），截断后用户看到的是 366 天，**默默丢失 34 天**
- 建议至少在导入时记一条日志（`VoiceTodoLog.calendar.warn`）+ 在 ConfirmSheet 给一行提示"事件过长，已截断到 366 天"

不强制改 UI，但不能默默截断。

### v1 与重复互斥的 UX 解释

Step 6 说"结束日期非空时禁用重复卡片，反之亦然，并给出一行说明文案"。Step 9 列了 `detail.span_recurrence_exclusive` key，但没说在哪显示。

**建议**：禁用时不只是视觉置灰，要在重复卡片下方加一行解释文字，绑定 `detail.span_recurrence_exclusive` 文案。否则用户不知道为什么开不了重复。

### 中间日通知不在范围 —— 应对产品追问的说辞

文档（行 307）说"跨天事件在中间日发通知不在 v1 范围"。但用户期望"周五到周日的出游"周六也提醒是合理的。

**若产品追问**：

- v1 完成语义是"整体一次完成"（已拍板），通知只在起始日排程与这套语义对齐
- 跨天中间日通知涉及"每天提醒但完成只算一次"的语义设计，单独一版做
- v1 用户想要中间日提醒可以手动给那几天设重复事件，绕过限制

不要在 v1 里临时塞中间日通知，会破坏整体完成语义。
