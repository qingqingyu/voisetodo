...
该文件从 `818536a` 起就是手写的,键名与 Xcode 生成的 `.storekit` 完全对不上。
...
| 商品 ID | `"id"` | `"productID"` |
| 续订周期 | `"subscriptionPeriod"` | `"recurringSubscriptionPeriod"` |
| 家庭共享 | `"familySharable"` | `"familyShareable"` |
| 商品 | 缺失 | `"internalID"`、`"groupNumber"`、`"type": "RecurringSubscription"` |
| 商品 | 多余臆造键 | `"levels"`、`"internal"`、`"recurringPrice"`、`"offerCode"`、`"name"` |
...
整体替换为下面的内容。product ID、价格(12 / 98)、3 天免费试用、中英双语文案全部保持不变,
只改 schema。
...
- `internalID` 和组 `id` 用 8 位十六进制(Xcode 的约定),**不要**用 UUID。
...
配合 3.2,第三屏首帧走 `loadingPlaceholder`(`PaywallView.swift:393-400`)。
...
- en:`Couldn't load plans. Please try again later.`

`.error` 分支改为 `subtitle: entitlement.lastError ?? String(localized: "paywall.products_load_failed")`,
...
3. 模拟器**先删除 App**(重置 `hasCompletedOnboarding`),重新 Run,走完前两屏到第三屏:
   预期先看到 spinner,随后出现 ¥98/年(默认选中)+ ¥12/月 两张卡、"3 天免费试用" 文案和订阅 CTA。
