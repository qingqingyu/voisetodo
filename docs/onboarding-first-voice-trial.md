...
| 「现状文件这一处本来就是对的」 | 现状 `paymentMode: "free"`（`:65` / `:117`），该文档当时论证的正确值是 `"freeTrial"` | **2026-08-22 实测推翻当时的论证**：v5 schema 免费试用就写 `"free"`，`"freeTrial"` 会让 runtime 解码整个配置失败（见 paywall-products-empty §3.1 修正说明）。即：现状文件这处本来就是对的，原表格此行的「结论」反了；后续 132a03f 依错误论证改成 `"freeTrial"` 才是 bug 引入点，已改回 |
