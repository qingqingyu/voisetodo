...
## 6. 待执行时补全的悬而未决点（review 补充 2026-08-08）

> 本节由 review 在 2026-08-08 补充。前面 1–5 节是已拍板的设计决策，本节是**执行前/执行中必须拍板**的具体问题。每条都附了建议默认值，执行者可以直接采纳或反馈。

### 6.1 Step 2 影响面清单的"需逐个核对"项（开工前必做）

第 153–164 行的表格列出了 7 个消费方，其中标注"**需逐个核对**"的几处必须在 Step 2 合入前逐行 audit：

- `AppCoordinator.swift:613 / 670 / 761 / 799`
- `CalendarSyncService.swift:170`

**判定标准**：该 `.first(where:)` / `.filter` 调用是否可能命中**完成于 90 天窗口之外**的 todo。

- 命中 → 改走 `TodoStore.findTodoItem(by:)`（`TodoStore.swift:770-773`，已带 `fetchLimit = 1`）
- 不命中 → 在代码注释里写明"该调用方仅处理工作集内的项"

漏一处 = 一个潜在 bug（用户编辑窗口外的已完成项时报"找不到 todo"）。

### 6.2 空态误判 —— 建议直接拍板"加一个 COUNT 查询"

原文（行 161）说"库里全是窗口外的已完成项时会误判为空态 → 实测确认；若需要，改用 `todos.isEmpty && !hasAnyCompleted` 之类的判据"。

**建议直接拍板**：加一条 `hasAnyCompletedInDb` 单条 SQL COUNT 查询（`modelContext.fetchCount(_:)`），空态判定改为 `todos.isEmpty && !hasAnyCompletedInDb`。

理由：

- 接受误判 = 一个已经用了几个月的用户清完所有待办后，看到空态会以为数据丢了
- COUNT 查询有 Step 1 的 `isCompleted` 索引兜底，开销可忽略
- 比留一个"实测后决定"的悬案干净

### 6.3 90 天窗口的选择依据

90 是临时拍板的数字，没有数据支撑。建议执行前：

1. 若有内测留存数据，看用户翻回看历史完成项的中位时间窗口
2. 没数据就先用 90 跑，在 `completedWindowDays` 上方写注释"v1 取保守值；若用户反馈'看不到上个月的完成项'再放宽"

不要纠结这个数字，它最终是可调常量。

### 6.4 Step 1 的旧库测试方法（执行者可能不知道怎么造旧库）

原文（行 86–88）要求"必须在模拟器上用**已有数据的旧库**实测"，但没说怎么造。具体步骤：

1. 在合并 Step 1 之前的 commit 上，用 `seedForUITests`（`TodoStore.swift:528-556`）或临时调试入口灌 100+ 条 todo（含未完成 + 已完成）
2. 退出 app，确认 `Library/Application Support/default.store` 存在
3. 切到带 `#Index` 的分支，重新跑 app
4. 重点观察：
   - 冷启动无崩溃、无 SwiftData 迁移错误日志
   - **Widget 侧 `readOnly()` 容器能正常打开**（这条是原文标注的唯一真实风险）
   - 进入 Review / Home 后数据齐全
