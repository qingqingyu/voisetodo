...
> 状态：**已实施**。提交 `9a894e7`（头号坑）+ `281db0d`（抽共享组件）+ `8acfde7`（面板/接线/本地化/抖动反馈）。实施时新增三个决策，见下方「实施时新增的决策」。手动视觉验收待 AI proxy 修复后补。
>
...
| 决策点 | 选择 | 原因 |
|---|---|---|
| 流式未结束时的展开行为 | **禁用展开**（非 carry over） | 流式帧会覆盖 `ExtractedTodo` 实例吃掉用户编辑。carry over 更对但要改合并逻辑；禁用展开最简，牺牲"流式中展开"这一边缘体验。流式结束后正常展开 |
| 流式期间点卡片的反馈 | **抖一下 + "解析中暂时不能编辑"提示 1.5s** | 禁用展开若静默，用户不知道为什么点不动。`ShakeEffect`（GeometryEffect）横向晃 ~2 次 + overlay 提示，作用域仅卡片头部不连带面板 |
| 卡片上的模糊时段 Menu | **删除**（时段统一进面板） | 原方案是"展开时隐藏 Menu"。实施时改方案 B：直接删，让标题/时段/日期/分类/优先级都统一在面板里改，交互一致（不用记"哪个要展开改、哪个不用"）。代价：时段失去原"未展开即可选"的快捷 |
...
> **已完成**：单元测试 ✅（16/16 通过）+ 构建 ✅（BUILD SUCCEEDED）+ 双 review 循环 3 次共 6 轮收敛（Three Check + code-review-expert 最终均 0 问题）。**待补**：手动视觉验收（卡在 AI proxy 503，proxy 修复后按下方清单验）。
