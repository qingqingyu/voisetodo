...
> 状态：**已实施**（2026-09-05，fupan `fdc13a5`→`73f1eae` 按落地顺序五批落地，含 tab 基线 `d53f380` 合并）。基线 `7959174`（`claude/review-flow-breakpoints-x8izga`）。
> **实施偏差（一处）**：⑤ 屏全零态不渲染 `summary_caption`——「你的每个决定,都在这串数字里」在零决定下自相矛盾(方案写「+既有 caption」,按其语义判断省略,见 `ReviewStepLedger.summaryCard` 注释)。
> **实施补充**：ja 补齐时发现 `review.chart.mark` 是空条目(a11y 会回落键名),顺带补全三语。真机手测 7 项(验证章)待做。
...
4. **② 屏三语按钮**：中/英/日下四个按钮标签都完整可读。三语里最危险的是 **en**（「Not doing it」12 字符、「Next week」9 字符都在两侧，而 `layoutPriority(1)` 给的是中段两钮——塌陷风险在两侧）。2026-09-07 修正：原写「日文『今日やる』最长，重点看它」是错的（ja 最长仅 4 字符），详见 `todo-review-flow-v3-implementation-review.md` 发现 5。
5. **③ 屏两条路径**：有洞察 → 正常出；真空态（无洞察、无占位、无阶梯提示三块全空——实施审阅发现 1 修复后跳过条件收窄，ranked 空但有占位**或阶梯提示**时第 3 步**照常出**）→ 第 2 步 Next 直达 ④ 屏，**无闪屏**（引擎前移的验收点）。
